<x-app-layout>
    <div class="h-[calc(100svh-0px)] md:h-[calc(100vh-64px)] bg-[#050505] relative overflow-hidden text-white font-sans selection:bg-indigo-500/30 flex" 
        ...
        <div class="flex-1 relative bg-black overflow-hidden flex items-center justify-center transition-all duration-500">
                ...
                <div class="bg-black/40 backdrop-blur-2xl p-2 pr-6 rounded-2xl border border-white/10 flex items-center gap-3 shadow-2xl">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-1.5 mt-0.5 opacity-80">
                            ...
                            <span class="text-[8px] font-black uppercase tracking-widest" x-text="partnerState === 'active' ? partnerData?.rank_name : (partnerState === 'away' ? 'Вне вкладки' : 'Потеря связи...')"></span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- ВИДЕО -->
            <video x-ref="remoteVideo" autoplay playsinline webkit-playsinline class="w-full h-full object-cover transition-all duration-700" :class="isRemoteBlurred ? 'blur-[100px] scale-110 opacity-50' : 'opacity-100'"></video>
            ...
            <div x-show="state === 'searching' || state === 'idle'" class="absolute inset-0 flex flex-col items-center justify-center bg-[#050505] z-[110]">
                ...
                <h3 class="text-white font-black uppercase text-[9px] tracking-[0.4em]" x-text="state === 'searching' ? 'Поиск собеседника...' : 'Нажми старт для общения'"></h3>
            </div>

            <!-- СВОЁ ВИДЕО (PIP) -->
            <div x-show="showSelfVideo" class="absolute bottom-40 md:bottom-28 md:left-8 right-6 w-28 md:w-56 aspect-[3/4] md:aspect-video bg-[#111] rounded-3xl overflow-hidden shadow-2xl border border-white/10 z-[80]">
                ...
                <div class="pointer-events-auto flex flex-col items-center gap-3 w-full max-w-lg">
                    ...
                    <div x-show="controlsOpen" x-transition class="flex items-center gap-1.5 p-1.5 bg-black/60 backdrop-blur-3xl border border-white/10 rounded-2xl">
                        ...
                        <template x-if="state === 'connected'">
                            <div class="flex items-center gap-1.5">
                                ...
                                <button @click="toggleContact()" :class="isFriend ? 'bg-green-600' : 'bg-white/5'" class="h-10 md:h-12 px-4 rounded-xl font-black text-[9px] uppercase tracking-widest"><span x-text="isFriend ? 'Друг ✓' : '+ Друг'"></span></button>
                    ...
                    <div class="w-full bg-[#121212]/90 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] p-2 flex items-center justify-between shadow-2xl">
                        ...
                        <div class="flex-1 flex justify-center px-4">
                            <template x-if="state === 'idle'"><button @click="startSearch()" class="bg-indigo-600 w-full h-12 rounded-full font-black text-[10px] uppercase shadow-lg">Начать поиск</button></template>
                            ...
                            <template x-if="state === 'connected'">
                                <div class="flex items-center gap-2 w-full">
                                    <button @click="stopSearch()" class="bg-red-600/20 text-red-500 px-6 h-12 rounded-full font-black text-[10px] uppercase">Стоп</button>
                                    <button @click="startSearch()" class="bg-white text-black flex-1 h-12 rounded-full font-black text-[10px] uppercase shadow-xl">Далее ➔</button>
                                </div>
                            </template>
                        </div>
            ...
            <div class="bg-[#0f0f0f] border border-white/10 w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl">
                ...
                <div class="space-y-6">
                    <template x-for="kind in ['video', 'audio']">
                        <div>
                            <label class="text-[9px] font-black uppercase text-gray-500 mb-2 block" x-text="kind === 'video' ? 'Камера' : 'Микрофон'"></label>
                            <div class="max-h-40 overflow-y-auto space-y-2 custom-scrollbar">
                                <template x-for="d in devices.filter(x => x.kind === (kind + 'input'))">
                                    <button @click="switchDevice(kind, d.deviceId)" 
                                            ...
                                            x-text="d.label || 'Устройство'"></button>
                                </template>
                            </div>
                        </div>
                    </template>
