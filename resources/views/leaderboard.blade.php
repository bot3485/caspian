            ...
            <div class="space-y-3">
                @foreach($topUsers as $index => $u)
                    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 p-5 md:p-6 bg-[#050505]/40 backdrop-blur-sm border rounded-[1.75rem] transition-all duration-300 group
                        ...
                        {{ $index === 1 ? 'border-slate-400/20 bg-gradient-to-r from-slate-400/[0.03] to-transparent shadow-[0_0_30px_rgba(148,163,184,0.02)]' : '' }}
                        {{ $index === 2 ? 'border-amber-700/20 bg-gradient-to-r from-amber-700/[0.03] to-transparent shadow-[0_0_30px_rgba(180,83,9,0.02)]' : '' }}
                        {{ $index > 2 ? 'border-white/[0.03] hover:border-white/10 hover:bg-[#080808]/60' : '' }}">
                        ...
                        <div class="flex items-center gap-4 flex-1 w-full">
                            ...
                            <div class="w-12 h-12 rounded-xl flex items-center justify-center font-black text-lg text-white group-hover:scale-105 transition-all duration-300 shrink-0 border
                                {{ $index === 0 ? 'bg-gradient-to-br from-amber-500 to-amber-600 border-amber-400/30 shadow-[0_0_15px_rgba(245,158,11,0.2)]' : '' }}
                                {{ $index === 1 ? 'bg-gradient-to-br from-slate-500 to-slate-600 border-slate-400/30 shadow-[0_0_15px_rgba(148,163,184,0.2)]' : '' }}
                                {{ $index === 2 ? 'bg-gradient-to-br from-amber-700 to-amber-800 border-amber-700/30 shadow-[0_0_15px_rgba(180,83,9,0.2)]' : '' }}
                        ...
                        <!-- Очки опыта и Время в эфире (Английская локализация) -->
