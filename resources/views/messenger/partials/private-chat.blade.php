<div x-show="activeFriend" class="flex-1 flex flex-col overflow-hidden relative bg-[#050505]" x-cloak>
    ...
    <div class="px-4 py-3 bg-[#020202]/90 backdrop-blur-2xl border-b border-white/[0.05] flex items-center justify-between z-20">
        <div class="flex items-center gap-3">
            ...
            <div class="flex flex-col">
                ...
                <!-- Индикатор печати перемещен в заголовок для профессионального вида -->
                <div class="h-3">
    ...
    <div class="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 custom-scrollbar flex flex-col" x-ref="friendChatBox">
        <template x-for="(msg, index) in friendMessages" :key="msg.id || index">
            <div class="w-full flex flex-col" :class="String(msg.sender_id) === String(myHashid) ? 'items-end' : 'items-start'">
                ...
                <!-- System Handshake Message -->
                <template x-if="msg.message === 'SYSTEM_FRIEND_REQUEST'">
                    <div class="w-full flex justify-center my-6">
                        <div class="bg-amber-500/5 border border-amber-500/20 p-6 rounded-[2.5rem] flex flex-col items-center max-w-[280px] text-center">
                            <div class="w-12 h-12 bg-amber-500/20 rounded-2xl flex items-center justify-center text-xl mb-3">🤝</div>
                            <span class="text-[10px] font-black uppercase tracking-[0.2em] text-amber-500 mb-4">{{ __('messenger.Incoming_Protocol') }}</span>
                            
                            <div class="flex gap-2" x-show="String(msg.sender_id) !== String(myHashid)">
                                <button @click="handleFriendRequest(activeFriend.id, 'accept')" class="px-6 py-2.5 bg-brand-indigo text-white rounded-xl text-[9px] font-black uppercase hover:scale-105 transition-all shadow-lg">{{ __('messenger.Accept') }}</button>
                                <button @click="handleFriendRequest(activeFriend.id, 'decline')" class="px-6 py-2.5 bg-white/5 text-gray-500 rounded-xl text-[9px] font-black uppercase hover:bg-white/10 transition-all">{{ __('messenger.Decline') }}</button>
                            </div>
                            
                            <span x-show="String(msg.sender_id) === String(myHashid)" class="text-[9px] font-bold text-gray-600 uppercase tracking-widest italic">Wait for user approval...</span>
                        </div>
                    </div>
                </template>

                <!-- System Accepted Message -->
    ...
    <div class="p-4 bg-[#020202]/80 backdrop-blur-xl border-t border-white/5">
        <!-- Блокируем ввод, если дружба еще не принята -->
