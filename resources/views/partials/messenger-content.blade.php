...
<div class="flex-1 flex flex-col overflow-hidden bg-[#050505]">
    ...
    <div x-show="tab === 'chat' && !activeFriend" class="flex-1 flex flex-col overflow-hidden">
        <template x-if="isCallingFriend && state === 'connected'">
            <div class="flex-1 flex flex-col items-center justify-center p-10 text-center space-y-4">
                ...
                <p class="text-[10px] text-gray-500 leading-relaxed">Вы в приватном звонке. Перейдите в чат к другу.</p>
                <button @click="openFriendChat(partnerData)" class="bg-indigo-600 px-6 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest active:scale-95 transition-all">Открыть чат</button>
            </div>
        </template>
                ...
                <div class="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar" x-ref="chatBox">
                    <template x-for="msg in messages" :key="msg.timestamp">
                        <div :class="msg.isMe ? 'items-end' : 'items-start'" class="flex flex-col">
                            <div :class="msg.isMe ? 'bg-indigo-600 shadow-lg' : 'bg-white/5 border border-white/10'" 
        ...
        <template x-if="state !== 'connected'">
            <div class="flex-1 flex flex-col items-center justify-center opacity-30 p-10 text-center">
                ...
                <div class="text-[10px] font-black uppercase tracking-widest">Чат появится после соединения</div>
            </div>
        </template>
    </div>
    ...
    <div x-show="activeFriend" class="flex-1 flex flex-col overflow-hidden" x-transition>
        <div class="p-4 bg-[#0a0a0a] border-b border-white/5 flex items-center justify-between shrink-0">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <div class="text-[7px] font-bold uppercase tracking-tighter" :class="activeFriend?.is_online ? 'text-green-500' : 'text-gray-500'" x-text="activeFriend?.is_online ? 'Онлайн' : 'Офлайн'"></div>
                </div>
            </div>
            <button @click="callFriend(activeFriend)" :disabled="!activeFriend?.is_online"
                    :class="activeFriend?.is_online ? 'bg-indigo-600 shadow-lg' : 'bg-white/5 opacity-20'"
                    class="px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">📞 Звонок</button>
        </div>
        ...
        <div class="p-4 bg-[#0a0a0a] border-t border-white/5 pb-safe shrink-0">
            <div class="flex gap-2 bg-black/40 p-2 rounded-2xl border border-white/5">
                <input type="text" x-model="friendChatInput" @input="sendTypingSignal()" @keyup.enter="sendFriendMsg()" placeholder="Сообщение..." class="flex-1 bg-transparent border-none text-sm focus:ring-0 px-4 h-12 text-white">
                <button @click="sendFriendMsg()" class="bg-indigo-600 text-white w-12 h-12 rounded-xl hover:bg-indigo-500">➔</button>
            </div>
        </div>
    </div>
        ...
        <template x-for="h in historyList" :key="h.id">
            <div class="p-4 bg-white/[0.03] border border-white/5 rounded-[2rem] flex flex-col gap-4 group hover:border-indigo-500/30 transition-all">
                <div class="flex items-center justify-between">
                    ...
                    <button @click="if(confirm('Заблокировать?')) { window.axios.post('/chat/block', {userId: h.id}).then(() => loadHistory()) }" class="w-10 h-10 flex items-center justify-center rounded-xl bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white transition-all">🚩</button>
                </div>

                <div class="flex gap-2">
                    <button @click="openFriendChat(h)" class="flex-1 bg-white/5 hover:bg-indigo-600 py-3 rounded-xl text-[9px] font-black uppercase tracking-widest text-white transition-all">💬 Написать</button>
                    
                    <template x-if="!h.is_friend && !h.is_blocked">
                        <button @click="window.axios.post('/chat/contact/add', {contactId: h.id}).then(() => { h.is_friend = true; window.dispatchEvent(new CustomEvent('toast', {detail: {msg: 'Добавлен в друзья', type: 'success'}})) })" class="px-4 bg-white/5 hover:bg-green-600/20 hover:text-green-500 rounded-xl text-lg transition-all">+</button>
                    </template>
                </div>
            </div>
        </template>
        <template x-if="historyList.length === 0">
            <div class="text-center py-20 opacity-20"><p class="text-[10px] font-black uppercase">История пуста</p></div>
        </template>
    </div>
