<div class="fixed bottom-24 md:bottom-8 left-0 right-0 px-6 z-[500] flex flex-col items-center gap-5 pointer-events-none"
    ...
    <div x-show="controlsOpen" 
         ...
         class="pointer-events-auto w-full max-w-[380px] led-frame rounded-[2.5rem] shadow-2xl">
        
        <!-- Внутренний слой: полностью блокирует свет внутри, оставляя 2px рамку снаружи -->
        <div class="led-content !bg-[#0a0a0a] p-4 relative border border-white/[0.04] shadow-[inset_0_2px_20px_rgba(255,255,255,0.02)]">
    ...
    <!-- MAIN ACTION BAR -->
    <!-- Внешний led-frame меняет форму от круга до широкой пилюли -->
    <div class="pointer-events-auto transition-all duration-[600ms] ease-[cubic-bezier(0.34,1.56,0.64,1)] led-frame shadow-2xl"
         :class="actionsOpen ? 'w-full max-w-[420px] rounded-[3rem]' : 'w-16 h-16 rounded-full hover:scale-110'">
        
        <!-- Внутренний слой: непроницаем для LED -->
        <div class="led-content flex items-center transition-all duration-[600ms] !bg-[#0a0a0a] border border-white/[0.04] shadow-[inset_0_2px_20px_rgba(255,255,255,0.02)]"
             :class="actionsOpen ? 'p-2 w-full h-full rounded-[3rem]' : 'w-full h-full rounded-full'">
            ...
            <div class="flex-1 flex justify-center px-3 overflow-hidden transition-all duration-500" x-show="actionsOpen">
                ...
                <template x-if="callContext === 'personal'">
                    <div class="w-full">
                        <button @click="stopCall()" class="w-full py-4 bg-red-600 text-white rounded-full font-black text-[11px] uppercase tracking-widest shadow-[0_0_20px_rgba(220,38,38,0.4)] hover:bg-red-500 transition-colors">
