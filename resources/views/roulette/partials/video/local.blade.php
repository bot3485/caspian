<div @click="toggleFocus('local')"
    ...
    <div class="relative w-full h-full rounded-[2.4rem] overflow-hidden bg-[#050505] shadow-[inset_0_0_30px_rgba(0,0,0,0.8)] z-10">
        ...
        <div class="absolute top-5 left-5 md:top-6 md:left-6 z-40" x-data="{ expanded: false }" @click.away="expanded = false">
            <button @click.stop="expanded = !expanded" class="px-4 py-2.5 bg-black/60 backdrop-blur-xl rounded-[1rem] flex items-center gap-2.5 rgb-led-border border-transparent hover:border-brand-indigo/40 hover:bg-white/10 transition-all shadow-xl group pointer-events-auto">
